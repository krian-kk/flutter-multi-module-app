class LoginResponseModel {
	...
	LoginResponseModel({this.code, this.status, this.msg, this.auth, this.data,  this.path, this.bank, this.region, this.area, this.contractor, this.summary, this.usageSummary, this.searchKey, this.searchValue, this.totalCaseCount, this.totalsEventObj});
	...
	LoginResponseModel.fromJson(Map<String, dynamic> json) {
		...
		if (json['bank'] != null) {
			bank = [];
			json['bank'].forEach((v) { bank?.add(json['bank']); });
		}
		if (json['region'] != null) {
			region = [];
			json['region'].forEach((v) { region?.add(json['region']); });
		}
		if (json['area'] != null) {
			area = [];
			json['area'].forEach((v) { area?.add(json['area']); });
		}
		if (json['contractor'] != null) {
			contractor = [];
			json['contractor'].forEach((v) { contractor?.add(json['contractor']); });
		}
		summary = json['summary'] != null ? Casecount.fromJson(json['summary']) : null;
		usageSummary = json['usageSummary'] != null ? Casecount.fromJson(json['usageSummary']) : null;
		searchKey = json['searchKey'];
		searchValue = json['searchValue'];
		totalCaseCount = json['totalCaseCount'];
		totalsEventObj = json['totalsEventObj'] != null ? Casecount.fromJson(json['totalsEventObj']) : null;
	}
...
class Data {
	...
	Data({this.accessToken, this.expiresIn, this.refreshExpiresIn, this.refreshToken, this.tokenType, this.notBeforePolicy, this.sessionState, this.scope, this.resetFlag, this.keycloakId});
