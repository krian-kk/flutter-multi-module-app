...
import 'package:origa/src/features/case_details_screen/address_details_bottomsheet_screen.dart';
import 'package:origa/src/features/case_details_screen/address_screen/address_screen.dart';
...
class _CaseDetailsScreenState extends State<CaseDetailsScreen> {
  ...
  void initState() {
    super.initState();
    bloc = BlocProvider.of<CaseDetailsBloc>(context)
    ...
    return WillPopScope(
      onWillPop: () {
        Navigator.pop(
          context,
          <String, dynamic>{
            'isSubmit': bloc.isEventSubmitted,
            'caseId': bloc.caseId!,
            'isSubmitForMyVisit': bloc.isSubmittedForMyVisits,
            'eventType': bloc.submittedEventType,
            'returnCaseAmount': bloc.caseDetailsAPIValue.caseDetails?.due,
            ...
            'selectedClipValue':
                (Singleton.instance.usertype == Constants.telecaller)
                    ...
                    : bloc.caseDetailsAPIValue.caseDetails?.collSubStatus,
        ...
        body: BlocListener<CaseDetailsBloc, CaseDetailsState>(
          ...
          listener: (BuildContext context, CaseDetailsState state) {
            ...
            if (state is UpdateRefUrlState) {
              ...
              bloc.caseDetailsAPIValue.caseDetails?.repaymentInfo?.refUrl =
                  state.refUrl;
              setState(() {});
            }
                            ...
                            if (value == 'IconEnum.back') {
                              Navigator.pop(
                                ...
                                <String, dynamic>{
                                  ...
                                  'caseId': bloc.caseId!,
                                  'isSubmitForMyVisit':
                                      bloc.isSubmittedForMyVisits,
                                  'eventType': bloc.submittedEventType,
                                  'returnCaseAmount':
                                      bloc.caseDetailsAPIValue.caseDetails?.due,
                                  'returnCollectionAmount':
                                      bloc.collectionAmount,
                                  'selectedClipValue':
                                      (Singleton.instance.usertype ==
                                              Constants.telecaller)
                                          ? bloc.caseDetailsAPIValue.caseDetails
                                              ?.telSubStatus
                                          : bloc.caseDetailsAPIValue.caseDetails
                                              ?.collSubStatus,
                                  'followUpDate': bloc.changeFollowUpDate,
    ...
    return Stack(
      children: <Widget>[
        ...
        if (Singleton.instance.usertype == Constants.fieldagent)
          bloc.caseDetailsAPIValue.caseDetails?.collSubStatus == 'new'
              ? Padding(
        ...
        if (Singleton.instance.usertype == Constants.telecaller)
          bloc.caseDetailsAPIValue.caseDetails?.telSubStatus == 'new'
              ? Padding(
                  padding: const EdgeInsets.only(left: 10),
                  child: CaseStatusWidget.satusTextWidget(
                    context,
              ...
              : Padding(
                  padding: const EdgeInsets.only(left: 10),
                  child: caseStatusWidget(
                      text: bloc.caseDetailsAPIValue.caseDetails?.telSubStatus),
                ),
      ],
    );
  }
  ...
  Widget basicInfo() {
    Map<String, dynamic>? caseResult =
        bloc.caseDetailsAPIValue.bucketedCaseData ?? {};
    ...
    if (bloc.caseDetailsAPIValue.otherLoanDetails?.isNotEmpty == true) {
    ...
    //                 title: Languages.of(context)!.repaymentInformation),
    //             const SizedBox(height: 20),
    //             if (bloc.caseDetailsAPIValue.caseDetails?.attr !=
    //                     null &&
    //                 bloc.caseDetailsAPIValue.caseDetails?.attr!
    //                         .length !=
    //                     0)
    //               Column(
    //                 children: [
    ...
    //                 shrinkWrap: true,
    //                 itemCount: bloc.caseDetailsAPIValue.otherLoanDetails
    //                         ?.length ??
    //                     0,
    ...
    return Column(
      ...
      children: [
        ...
        ListView.builder(
            ...
            padding: EdgeInsets.zero,
            shrinkWrap: true,
            itemCount: bloc.caseDetailsAPIValue.otherLoanDetails?.length ?? 0,
            itemBuilder: (BuildContext context, int index) {
              return Column(
                mainAxisSize: MainAxisSize.min,
                children: <Widget>[
                  ...
                  GestureDetector(
                    onTap: () {
                      if (bloc.caseDetailsAPIValue.otherLoanDetails![index]
                          .canAccess!) {
                        bloc.add(ClickPushAndPOPCaseDetailsEvent(
                            paramValues: <String, dynamic>{
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: <Widget>[
                            ...
                            const SizedBox(height: 5),
                            CustomText(
                              bloc.caseDetailsAPIValue.otherLoanDetails![index]
                                          .cust !=
                                      null
                                  ? bloc.caseDetailsAPIValue
                                      .otherLoanDetails![index].cust!
                    ...
                    child: GestureDetector(
                      onTap: () {
                        bloc.add(SendWhatsAppEvent(context,
                            caseID:
                                bloc.caseDetailsAPIValue.caseDetails!.caseId!));
                      },
                      child: Container(
                        padding: const EdgeInsets.symmetric(
                            horizontal: 15, vertical: 11.2),
                        ...
                        child: GestureDetector(
                          onTap: () async {
                            ...
                            bloc.add(GeneratePaymenLinktEvent(context,
                                caseID: bloc
                                    .caseDetailsAPIValue.caseDetails!.caseId!));
                          },
                          child: Container(
                            padding: const EdgeInsets.symmetric(
        ...
        builder: (BuildContext context) {
          return PhoneScreen(
            ...
            customerLoanUserWidget: CustomLoanUserDetails(
              ...
              userAmount:
                  bloc.caseDetailsAPIValue.caseDetails?.due?.toDouble() ?? 0.0,
            ),
          );
        });
  }
        ...
        switch (cardTitle) {
          ...
          case Constants.collections:
            return CustomCollectionsBottomSheet(
              ...
              postValue: list[bloc.indexValue!],
              bloc: bloc,
              custName: bloc.caseDetailsAPIValue.caseDetails?.cust ?? '',
              isCallFromCaseDetails: isCallFromCallDetails,
              callId: callId,
            );
          case Constants.ots:
          ...
          case Constants.repo:
            return CustomRepoBottomSheet(
              ...
              customerLoanUserWidget: CustomLoanUserDetails(
                userName: bloc.caseDetailsAPIValue.caseDetails?.cust ?? '',
                userId:
                    '${bloc.caseDetailsAPIValue.caseDetails?.bankName} / ${bloc.caseDetailsAPIValue.caseDetails?.agrRef}',
                userAmount:
                    bloc.caseDetailsAPIValue.caseDetails?.due?.toDouble() ??
                        0.0,
              ),
          ...
          case Constants.callCustomer:
            ...
            bloc.caseDetailsAPIValue.callDetails?.forEach((dynamic element) {
            ...
            return CallCustomerBottomSheet(
              ...
              caseId: bloc.caseId != null
                  ...
                  : bloc.caseDetailsAPIValue.caseDetails!.caseId!,
              custName: bloc.caseDetailsAPIValue.caseDetails?.cust ?? '',
              sid: bloc.caseDetailsAPIValue.caseDetails!.id.toString(),
          ...
          children: <Widget>[
            Column(
              ...
              children: <Widget>[
                GestureDetector(
                  onTap: () async {
                    final String urlValue = bloc.caseDetailsAPIValue.caseDetails
                  ...
                  child: ListOfCaseDetails.textFieldView(
                      ...
                      value: bloc.caseDetailsAPIValue.caseDetails?.repaymentInfo
                              ?.refUrl ??
                          '-'),
                ),
                const SizedBox(height: 15),
  ...
  playAudio(String? audioPath, int index) async {
    ...
    if (postResult[Constants.success]) {
      final AudioConvertModel audioConvertData =
