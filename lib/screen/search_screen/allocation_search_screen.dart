...
class AllocationSearchScreen extends StatefulWidget {
  final AllocationBloc? bloc;
  const AllocationSearchScreen({Key? key, required this.bloc})
      : super(key: key);
  @override
  _AllocationSearchScreenState createState() => _AllocationSearchScreenState();
}

class _AllocationSearchScreenState extends State<AllocationSearchScreen> {
  bool isLoaded = false;
    ...
    return Scaffold(
      ...
      body: BlocListener<AllocationBloc, AllocationState>(
        ...
        listener: (context, state) {
          if (state is SearchAllocationScreenLoadedState) {
            setState(() {
              isLoaded = true;
            });
          }
          if (state is SearchAllocationScreenSuccessState) {
            setState(() => isLoaded = false);
            Navigator.pop(context);
          }
          if (state is SearchAllocationFailedState) {
            setState(() => isLoaded = false);
          ...
          child: CustomButton(
            ...
            onTap: () {
              if (accountNoController.text.isNotEmpty ||
                ...
                widget.bloc!.add(ClickAllocationSearchButtonEvent(
